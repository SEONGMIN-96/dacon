...
fname_train_data = 'train_data.npy'
fname_target = 'train_target.npy'
fname_test_data = 'test_data.npy'
fname_sample_train = 'sample_train.npy'
fname_sample_test = 'sample_test_data.npy'
...
for text in tqdm.tqdm(train[0]):
    try:
        clean_train_text.append(preprocessing(text, okt, remove_stopwords=True, stop_words=stop_words))
    except:
        clean_train_text.append([])

for text in tqdm.tqdm(test[0]):
    if type(text) == str:
...
x_train, x_test, y_train, y_test = train_test_split(train_feature, train[1],
