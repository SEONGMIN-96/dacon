...
with open('./dacon/_data/_npy/_classifier01/inputs.pkl','rb') as f :
...
for train_idx, valid_idx in skf.split(train_inputs[0], labels):
    ...
    ckpt_path = f'./dacon/_data/_save/mcp/_classifier01/{now}.ckpt'
    ...
    np.save(f'./dacon/_data/_save/mcp/_classifier01/{now}_prediction.npy', prediction)

predictions = []
for ar in glob('./dacon/_data/_save/mcp/_classifier01/*.npy'):
    arr = np.load(ar)
    predictions.append(arr)

sample = pd.read_csv('./dacon/_data/classifier01/sample_submission.csv')
sample['label'] = np.argmax(np.mean(predictions,axis=0), axis = 1)
sample.to_csv('./dacon/_data/_save/_classifier01/konlpy.csv',index=False)
